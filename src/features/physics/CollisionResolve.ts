...
import { NormalResolve } from "../../utils/math/CalculateNormal";
...
export class CollisionResolve {
    private normal: NormalResolve = new NormalResolve();
    private implulse: ImpulseResolve = new ImpulseResolve();
    private friction: FrictionResolve = new FrictionResolve();
    private position: PositionCorrectionResolve = new PositionCorrectionResolve();
    public resolvePlanetWithBox(planet: Planet, gameBox: GameBox) {
        this.applyResolvePlanetwithBox(planet, gameBox);
    }
    public resolvePlanetWithPlanet(planet1: Planet, planet2: Planet) {
        const normal = this.normal.calculateNormal(planet1, planet2);

        if (!normal) return;
        this.applyPositionCorrection(planet1, planet2, normal);

        const j = this.applyingImpulse(planet1, planet2, normal);
    ...
    private applyResolvePlanetwithBox(planet: Planet, gameBox: GameBox) {
